...
import { G as GH_USER_REPO, b as GH_PUBLISHED_TAGS, A as APPROVED_POSTERS_GH_USERNAME, R as REPO_OWNER } from "./siteConfig.js";
...
function slugify(text) {
  return text.toString().normalize("NFKD").toLowerCase().trim().replace(/\s+/g, "-").replace(/[^\w-]+/g, "").replace(/--+/g, "-").replace(/(^-|-$)/g, "");
}
function readingTime(text) {
  let minutes = Math.ceil(text.trim().split(" ").length / 225);
  return minutes > 1 ? `${minutes} minutes` : `${minutes} minute`;
}
async function listContent(providedFetch) {
  let _allBlogposts = [];
  let next = null;
  let limit = 0;
...
async function getContent(providedFetch, slug) {
...
function parseIssue(issue) {
  ...
  return {
    type: "blog",
    // futureproof in case you want to add other types of content
    content,
    frontmatter: data,
    title,
    ...
    // all posts assumed to be "note"s unless otherwise specified
    tags,
    image: data.image ?? data.cover_image,
    canonical: data.canonical,
    // for canonical URLs of something published elsewhere
